...
<body>
    ...
    <table>
        <thead>
            <tr><th>Item Details</th><th class="center">CONFIDENCE<br>SCORE</th><th>Unit Cost x Qty</th><th>Min</th><th>Avg</th><th>Max</th></tr>
        </thead>
        <tbody>
...
def calculate_estimate_data(payload):
    ...
    for item in materials:
        ...
        if labor_req:
            processed_items.append({"is_header": False, "type": "Labor", "conf_pct": lab_conf, "conf_label": get_conf_label(lab_conf), "u_cost": "-", "min": f"{lab_avg*0.85:,.2f}", "avg": f"{lab_avg:,.2f}", "max": f"{lab_avg*1.25:,.2f}"})
    ...
    return {"date": datetime.datetime.now().strftime("%B %d, %Y"), "project_type": project_info.get('projectType', 'N/A'), "quality_level": q_level, "zip_code": project_info.get('zipCode', 'N/A'), "items": processed_items, "mat_sub_min": f"{mat_sub[0]:,.2f}", "mat_sub_avg": f"{mat_sub[1]:,.2f}", "mat_sub_max": f"{mat_sub[2]:,.2f}", "lab_sub_min": f"{lab_sub[0]:,.2f}", "lab_sub_avg": f"{lab_sub[1]:,.2f}", "lab_sub_max": f"{lab_sub[2]:,.2f}", "tax_rate_display": f"{tax_rate*100:.2f}", "tax_min": f"{tax_arr[0]:,.2f}", "tax_avg": f"{tax_arr[1]:,.2f}", "tax_max": f"{tax_arr[2]:,.2f}", "tot_min": f"{tot_arr[0]:,.2f}", "tot_avg": f"{tot_arr[1]:,.2f}", "tot_max": f"{tot_arr[2]:,.2f}"}
...
def generate_report():
    try:
        ...
        items_list = report_data.get('items', [])
        logo_b64 = ""
